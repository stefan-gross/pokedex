...
export interface PixelMetrics {
  ...
  sharpness: number;
  /** Anteil (0..1) nahezu ausgebrannter Pixel (Luminanz ≥ 248) = Reflexion. */
  glare: number;
...
export const QUALITY_THRESHOLDS = {
  ...
  glareMax: 0.06,     // > 6 % ausgebrannt → Reflexion
  lumMin: 55,         // < → zu dunkel
...
export function computePixelMetrics(data: Uint8ClampedArray, w: number, h: number): PixelMetrics {
  const N = w * h;
  if (N === 0) return { sharpness: 0, glare: 0, meanLum: 0, contrast: 0 };

  const gray = new Float32Array(N);
  const hist = new Uint32Array(32);
  let sum = 0, glareCount = 0;
  for (let i = 0; i < N; i++) {
    const o = i * 4;
  ...
  return { sharpness, glare: glareCount / N, meanLum, contrast };
}
