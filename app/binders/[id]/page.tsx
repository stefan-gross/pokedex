...
export default function BinderDetailPage({ params }: Props) {
  ...
  const handleDelete = async () => {
    if (!binder) return;
    if (!confirm(`Mappe „${binder.name}" löschen?`)) return;
  ...
  const cols = binder.size === 9 ? 3 : binder.size === 12 ? 3 : binder.size === 16 ? 4 : 3;
  const slots = Array.from({ length: binder.size });
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Card grid */}
      <div className={`px-3 pb-6 grid gap-2`} style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
        {slots.map((_, i) => {
          const card = cards[i];
