...
| Scanner | Parallel-Scan: Kamera läuft durch, Gemini im Hintergrund; Cooldown 2s nach Snap; Pending-Badge; Gemini 2.5-flash + 1.5-flash Fallback bei 503; setId-Mapping SV01–SV09.5+SWSH+SM+XY+BW+Klassiker; Varianten-Erkennung (standard/holo/reverse/alt-art/promo) via Gemini; Duplikat-Prüfung via `getCardsByTcgId()` | `components/scanner/CameraCapture.tsx`, `app/api/scan/route.ts`, `components/scanner/CardScanResult.tsx`, `app/(app)/scanner/page.tsx` |
| Scanner — UX | Weißer Flash (180ms) nach Snap; Pause/Weiter-Toggle oben rechts; Zurück-Pfeil oben links; Scan-Ergebnisse als horizontaler Thumbnail-Slider unten; Papierkorb-Button pro Thumbnail; Duplikat-Badge (×N grün) auf Thumbnail + im Detail-Sheet | `app/(app)/scanner/page.tsx`, `components/scanner/CameraCapture.tsx` |
| Scanner — Rahmenerkennung | Echte Kontur-Erkennung via `detectCardContour()`: Pro Seite ganzes Band scannen (0–45%/55–100%), stärkste Kante finden, dann Seitenverhältnis ~1.4 prüfen (Pokémon-Karte) + Mindestfläche 10%; nur bei korrekter Form → gelb/grün; `SNAP_STABLE_FRAMES=3` (300ms Ruhe nötig); verhindert Fehlsnaps bei Kameraschwenks | `components/scanner/CameraCapture.tsx` |
| Scanner — UX (Fix) | Bottom Sheets (CardScanResult + CardDetailSheet) hatten z-50 = gleich wie BottomNav → Sheet war halb unter Nav versteckt; jetzt z-[60] für alle Sheets; Scanner-Slider Tiles: 54×76px → 72×101px (korrektes Kartenformat), Papierkorb als Badge oben-rechts außen | `app/(app)/scanner/page.tsx`, `components/card/CardDetailSheet.tsx` |
| Scanner — Stop-FAB & Review | Stop-FAB überlagert BottomNav-Kamera-Button sobald ≥1 Karte gescannt (z-[55], gleiche Position); grünes Badge mit Anzahl; Klick → Review-Modus: Kamera stoppt (CameraCapture unmountet), 3-spaltiges Karten-Grid mit Name, Hinzufügen-Button, grünem Haken nach Speichern; „+ Mehr" → zurück zu Scanning | `app/(app)/scanner/page.tsx` |
